...
process pang_to_bams {
    ...
    SqueezeMeta.pl -m coassembly -p ${pang_ID} -f ${fastq_dir} -s ${samples} -m extassembly -r ${pang_fasta} -t ${task.cpus} --nobins --norename -b ${block_size} -mapping_options "--ignore-quals --mp 1,1 --np 1 --rdg 0,1 --rfg 0,1 --score-min L,0,-0.05"
    sqm2zip.py ${pang_ID} .
    """
}
